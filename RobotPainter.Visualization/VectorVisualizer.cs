...
namespace RobotPainter.Visualization
{
    public static class VectorVisualizer
    {
        ...
        public static void VisualizeVectorsInline(Bitmap image, double[,] x, double[,] y, int n, Color arrow_c, Color dot_c, int arrow_length = 20, int tip_length = 3, int tip_angle = 30, int point_radius = 1)
        {
            int w = image.Width;
            int h = image.Height;
            int s_dim = w < h ? w : h;
